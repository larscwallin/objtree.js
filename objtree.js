 ...
 *
 *  Extended by Lars C Wallin to handle namespaces.
 *  Added optional semicolon substitution through the ns_separator property 
*/
...
(function(global) {
    ...
    XML.ObjTree.prototype.ns_separator = '_';
	...
	XML.ObjTree.prototype.parseElement = function ( elem ) {
	    ...
	    if ( elem.attributes && elem.attributes.length ) {
	        ...
	        for ( var i=0; i<elem.attributes.length; i++ ) {
				...
				if(key.indexOf(":") && this.ns_separator !== ""){
					key = key.replace(":",this.ns_separator,1);
				}
	    ...
	    if ( elem.childNodes && elem.childNodes.length ) {
	        ...
	        if ( textonly ) {
	            ...
	            for ( var i=0; i<elem.childNodes.length; i++ ) {
	                ...
	                val = this.parseElement( elem.childNodes[i] );
	                if ( ! val ) {continue;}
	                if ( typeof(cnt[key]) == "undefined" ){ cnt[key] = 0;}
	                cnt[key] ++;
	                this.addNode( retval, key, cnt[key], val );
	...
	XML.ObjTree.prototype.hash_to_xml = function ( name, tree ) {
		...
		if(name !== null && this.ns_separator !== "" && name.indexOf(this.ns_separator) > 0){
			name = name.replace(this.ns_separator,":",1);
		}
	    ...
	    for( var key in tree ) {
			...
			if(this.ns_separator !== "" && key.indexOf(this.ns_separator) > 0){
				key = key.replace(this.ns_separator,":",1);
			}
